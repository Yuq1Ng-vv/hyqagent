...
class TestRecordLlmCall:
    def test_records_and_reflects_in_output(self) -> None:
        m = _new_metrics()
        m.record_llm_call(
            model="gpt-4", phase="hypothesis_gen",
            input_tokens=500, output_tokens=200,
            cache_read_tokens=0, cost_usd=0.015,
            latency_seconds=1.2, status="success",
        )
        text = m.get_metrics_text()
    ...
    def test_multiple_calls_aggregate(self) -> None:
        ...
        for _ in range(3):
            m.record_llm_call(
                model="claude", phase="validation",
                input_tokens=100, output_tokens=50,
                cache_read_tokens=0, cost_usd=0.003,
                latency_seconds=0.5,
            )
        text = m.get_metrics_text()
        assert 'hyqagent_llm_calls_total' in text
    ...
    def test_different_statuses_labeled(self) -> None:
        ...
        m.record_llm_call(
            ...
            input_tokens=1, output_tokens=1,
            cache_read_tokens=0, cost_usd=0.0,
            latency_seconds=0.1, status="error",
        )
        text = m.get_metrics_text()
