...
- ✅ **[Phase 1]** **多语言 CPG 引擎**：tree-sitter 解析 + NetworkX 图 + def-use 数据流 + 跨文件调用图，支持 Python/JS/Java
- ✅ **[Phase 1]** **五种框架提取器**：Flask/Django/FastAPI/Express/Spring 路由自动发现，含认证检测
- ✅ **[Phase 1]** **9类别×3语言污点规则**：YAML 驱动，覆盖 SQL注入/命令注入/XSS/路径遍历/SSRF 等
- 📋 **[Phase 2+]** **五级危害 x 七层挖掘阶梯**：CRITICAL级（CVSS 9.0-10.0）7层全量挖掘到人工签字
- 📋 **[Phase 2+]** **单Agent + CPG + 多模型级联**：CPG按需提取代码切片，LLM做语义推理
- 📋 **[Phase 2+]** **200+细粒度检测项**：覆盖17大类，与OWASP ASVS v4/v5对齐
- 📋 **[Phase 4+]** **完整的可观测性与审计链**：SHA-256防篡改事件日志，每项发现可追溯

### 1.3 与现有工具对比

| 维度 | Semgrep/CodeQL | Bandit | HyqAgent（目标） | HyqAgent（Phase 1 实际） |
|:-----|:--------------|:-------|:---------|:---------|
| 分析方法 | AST模式匹配 | AST+预定义规则 | CPG数据流+LLM语义推理 | CPG数据流（确定性，零LLM） |
| IDOR/业务逻辑 | 完全静默 | 完全静默 | LLM语义检测（Phase 3+） | 框架级 auth 缺失检测 ✅ |
| 精确率 | 中 | 中-高 | 78.43%（设计目标） | 待 Phase 2-5 验证 |
| 幻觉控制 | N/A | N/A | L1+L2两层过滤（Phase 3+） | 纯确定性，零幻觉 ✅ |
| 成本/项目 | 零 | 零 | $1/$5/$25（设计预算） | $0（无 LLM 调用） |
| 语言支持 | 多语言 | Python | Python/JS/Java/多框架 | Python/JS/Java + 5框架 ✅ |
